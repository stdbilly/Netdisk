...
int rsa_generate_key(const char* user_name) {
    ...
    sprintf(cmd, "openssl rsa -in %s_rsa.key -pubout -out %s_rsa_pub.key",
            user_name, user_name);
    ret = access(pk_path, F_OK);
    if (ret) {
        system(cmd);
...
char* rsa_encrypt(const char* str) {
    ...
    ret = RSA_public_encrypt(len, (unsigned char*)str, (unsigned char*)en_str,
...
char* rsa_sign(char* str, const char* user_name) {
    ...
    ret = RSA_private_encrypt(len, (unsigned char*)str, (unsigned char*)en_str,
                              rsa, RSA_PKCS1_PADDING);
    if (ret == -1) {
...
char* rsa_decrypt(char* str, const char* user_name) {
    ...
    ret = RSA_private_decrypt(RSA_EN_LEN, (unsigned char*)str,
                              (unsigned char*)de_str, rsa, RSA_PKCS1_PADDING);
    if (ret == -1) {
        printf("rsa sign failed\n");
        fclose(fp);
        RSA_free(rsa);
...
char* rsa_verify(char* str) {
    ...
    ret = RSA_public_decrypt(SER_EN_LEN, (unsigned char*)str,
                             (unsigned char*)de_str, rsa, RSA_PKCS1_PADDING);
#ifdef DEBUG
    printf("de_strLen=%ld\n", strlen(de_str));
#endif
